    ...
    function scrapeDataFromDom(tabId) {
      return chrome.scripting.executeScript({
        ...
        func: () => {
          ...
          const usernames = Array.from(p_tags).filter((ele) => {
            if (ele.textContent.includes("@")) return true;
            else return false;
          }).map((user2) => {
            return user2.innerHTML;
          });
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
      if (message.type === "get-data") {
        (async () => {
          ...
          const [linksResult] = await chrome.scripting.executeScript({
            ...
            func: () => {
              function testValidDate(p) {
                return p.classList.contains("_text_bevez_41") && p.classList.contains("_shared_bevez_6") && p.classList.contains("_normal_bevez_51") && p.textContent === "Ship order";
              }
              function getInfo() {
                let receipts2 = Array.from(
                  document.getElementsByClassName(
                    "styles_receiptsListWrapper__bdK1V"
                  )
                );
                return receipts2.filter(
                  (ele) => Array.from(ele.getElementsByTagName("p")).some(
                    testValidDate
                  )
                );
              }
              const receipts = getInfo();
              const links = receipts.map((ele) => {
          ...
          const receiptLinks = linksResult.result;
